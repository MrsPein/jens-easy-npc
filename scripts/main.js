...
export async function createNPCActor(npcData, portraitPath) {
  ...
  // Add equipment items
  if (npcData.equipment?.length) {
    await addItemsToActor(actor, npcData.equipment);
  }
...
function buildActorData(npc, portraitPath) {
  ...
  return {
    ...
    system: {
      details: {
        biography: {
          value: buildBiography(npc)
        },
        race: npc.race ?? "",
        background: npc.background ?? "",
        alignment: npc.alignment ?? "true neutral",
        cr: npc.stats?.CR ?? 0,
        type: { value: "humanoid", subtype: npc.race ?? "" }
      },
      attributes: {
        hp: {
          value: npc.stats?.HP ?? 10,
          max: npc.stats?.HP ?? 10,
          formula: ""
        },
        ac: { flat: npc.stats?.AC ?? 10 },
        movement: {
          walk: parseSpeed(npc.stats?.speed),
          swim: npc.stats?.swimSpeed ?? 0,
          fly: npc.stats?.flySpeed ?? 0,
        }
      },
      abilities: {
        str: { value: npc.stats?.STR ?? 10 },
        dex: { value: npc.stats?.DEX ?? 10 },
        con: { value: npc.stats?.CON ?? 10 },
        int: { value: npc.stats?.INT ?? 10 },
        wis: { value: npc.stats?.WIS ?? 10 },
        cha: { value: npc.stats?.CHA ?? 10 }
      },
      traits: {
        languages: {
          value: parseLanguages(npc.stats?.languages ?? [])
        }
      }
    },
    flags: {
...
function buildBiography(npc) {
  ...
  if (npc.personality?.length) {
    lines.push(`<p><strong>Personality:</strong> ${npc.personality.join(". ")}</p>`);
  }
...
async function addItemsToActor(actor, equipmentNames) {
  // Try to find items by name in world items and compendiums
  const itemsToAdd = [];
  ...
  for (const name of equipmentNames) {
    ...
    if (worldItem) {
      itemsToAdd.push(worldItem.toObject());
      continue;
    }
    ...
    for (const pack of game.packs) {
      ...
      try {
        ...
        if (entry) {
          ...
          if (doc) { itemsToAdd.push(doc.toObject()); break; }
        }
      } catch(e) {}
    }
  }
...
function parseLanguages(languages) {
  const map = {
    ...
    "draconic": "draconic", "infernal": "infernal", "celestial": "celestial",
    "abyssal": "abyssal", "gnomish": "gnomish", "halfling": "halfling",
    "orc": "orc", "giant": "giant", "goblin": "goblin",
    "sylvan": "sylvan", "undercommon": "undercommon", "deep speech": "deep",
    "primordial": "primordial", "aquan": "aquan", "auran": "auran",
    "ignan": "ignan", "terran": "terran"
  };
  return languages
    .map(l => map[l.toLowerCase()] ?? l.toLowerCase())
    .filter(Boolean);
}
